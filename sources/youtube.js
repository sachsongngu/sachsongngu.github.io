...
var subtitlesContainer = document.querySelector('.subtitles-container'); // Giả sử bạn có một container cho các dòng phụ đề
...
function onYouTubeIframeAPIReady() {
    ...
    player.addEventListener('onStateChange', function(event) {
        if (event.data == YT.PlayerState.PLAYING) {
            ...
            // Chỉ cần gọi hàm autoScroll một lần khi video đang chạy
            autoScroll(); 
        }
    });
    ...
    function autoScroll() {
        ...
        // Tìm dòng active dựa trên currentTime
        subLines.forEach(function(subLine) {
            var startTime = parseFloat(subLine.getAttribute('data-start'));
            var endTime = parseFloat(subLine.getAttribute('data-end'));
            if (currentTime >= startTime && currentTime < endTime) {
        ...
        if (activeLine) {
            ...
            var scrollTop = activeLine.offsetTop - subtitlesContainer.offsetTop - 150;
